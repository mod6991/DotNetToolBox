                ...
                do
                {
                    ...
                    if (rec.Length > 0)
                    {
                        using (MemoryStream ms = new MemoryStream(rec))
                        {
                            ...
                            byte[] calcEnc = Cryptography.ChaCha20Rfc7539.Encrypt(data, key, iv);
                    ...
                    if (rec.Length > 0)
                    {
                        using (MemoryStream ms = new MemoryStream(rec))
                        {
                            ...
                            using (MemoryStream msIn = new MemoryStream(data))
                            {
                                ...
                                {
                                    Cryptography.ChaCha20Rfc7539.Encrypt(msIn, msOut, key, iv);
                ...
                do
                {
                    ...
                    if (rec.Length > 0)
                    {
                        using (MemoryStream ms = new MemoryStream(rec))
                        {
                            ...
                            byte[] calcDec = Cryptography.ChaCha20Rfc7539.Decrypt(enc, key, iv);
                    ...
                    if (rec.Length > 0)
                    {
                        using (MemoryStream ms = new MemoryStream(rec))
                        {
                            ...
                            using (MemoryStream msIn = new MemoryStream(enc))
                            {
                                using (MemoryStream msOut = new MemoryStream())
                                {
                                    Cryptography.ChaCha20Rfc7539.Decrypt(msIn, msOut, key, iv);
