                ...
                do
                {
                    ...
                    if (rec.Length > 0)
                    {
                        using (MemoryStream ms = new MemoryStream(rec))
                        {
                            ...
                            byte[] calcEnc = Cryptography.DES.EncryptCBC(data, key, iv);
                    ...
                    if (rec.Length > 0)
                    {
                        using (MemoryStream ms = new MemoryStream(rec))
                        {
                            ...
                            using (MemoryStream msIn = new MemoryStream(data))
                            {
                                ...
                                {
                                    Cryptography.DES.EncryptCBC(msIn, msOut, key, iv, Cryptography.PaddingStyle.None);
                ...
                do
                {
                    ...
                    if (rec.Length > 0)
                    {
                        using (MemoryStream ms = new MemoryStream(rec))
                        {
                            ...
                            byte[] calcDec = Cryptography.DES.DecryptCBC(enc, key, iv);
                    ...
                    if (rec.Length > 0)
                    {
                        using (MemoryStream ms = new MemoryStream(rec))
                        {
                            ...
                            using (MemoryStream msIn = new MemoryStream(enc))
                            {
                                using (MemoryStream msOut = new MemoryStream())
                                {
                                    Cryptography.DES.DecryptCBC(msIn, msOut, key, iv, Cryptography.PaddingStyle.None);
