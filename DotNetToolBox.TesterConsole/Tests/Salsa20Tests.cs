...
{
    internal static class Salsa20Tests
    {
                ...
                do
                {
                    ...
                    if (rec.Length > 0)
                    {
                        using (MemoryStream ms = new MemoryStream(rec))
                        {
                            ...
                            byte[] calcEnc = Cryptography.Salsa20.Encrypt(data, key, iv);
                    ...
                    if (rec.Length > 0)
                    {
                        using (MemoryStream ms = new MemoryStream(rec))
                        {
                            ...
                            using (MemoryStream msIn = new MemoryStream(data))
                            {
                                ...
                                {
                                    Cryptography.Salsa20.Encrypt(msIn, msOut, key, iv);
                ...
                do
                {
                    ...
                    if (rec.Length > 0)
                    {
                        using (MemoryStream ms = new MemoryStream(rec))
                        {
                            ...
                            byte[] calcDec = Cryptography.Salsa20.Decrypt(enc, key, iv);
                    ...
                    if (rec.Length > 0)
                    {
                        using (MemoryStream ms = new MemoryStream(rec))
                        {
                            ...
                            using (MemoryStream msIn = new MemoryStream(enc))
                            {
                                using (MemoryStream msOut = new MemoryStream())
                                {
                                    Cryptography.Salsa20.Decrypt(msIn, msOut, key, iv);
