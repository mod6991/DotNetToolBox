                ...
                do
                {
                    ...
                    if (rec.Length > 0)
                    {
                        using (MemoryStream ms = new MemoryStream(rec))
                        {
                            ...
                            byte[] calcEnc = Cryptography.TripleDES.Encrypt(data, key, iv);
                    ...
                    if (rec.Length > 0)
                    {
                        using (MemoryStream ms = new MemoryStream(rec))
                        {
                            ...
                            using (MemoryStream msIn = new MemoryStream(data))
                            {
                                ...
                                {
                                    Cryptography.TripleDES.Encrypt(msIn, msOut, key, iv, Cryptography.PaddingStyle.None);
                ...
                do
                {
                    ...
                    if (rec.Length > 0)
                    {
                        using (MemoryStream ms = new MemoryStream(rec))
                        {
                            ...
                            byte[] calcDec = Cryptography.TripleDES.Decrypt(enc, key, iv);
                    ...
                    if (rec.Length > 0)
                    {
                        using (MemoryStream ms = new MemoryStream(rec))
                        {
                            ...
                            using (MemoryStream msIn = new MemoryStream(enc))
                            {
                                using (MemoryStream msOut = new MemoryStream())
                                {
                                    Cryptography.TripleDES.Decrypt(msIn, msOut, key, iv, Cryptography.PaddingStyle.None);
