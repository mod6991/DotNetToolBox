...
{
    internal static class PaddingX923Tests
    {
                ...
                do
                {
                    ...
                    if (rec.Length > 0)
                    {
                        using (MemoryStream ms = new MemoryStream(rec))
                        {
                            ...
                            byte[] calcPadded = Cryptography.Padding.Pad(data, 16, Cryptography.PaddingStyle.AnsiX923);
                ...
                do
                {
                    ...
                    if (rec.Length > 0)
                    {
                        using (MemoryStream ms = new MemoryStream(rec))
                        {
                            ...
                            byte[] calcUnpadded = Cryptography.Padding.Unpad(padded, 16, Cryptography.PaddingStyle.AnsiX923);
